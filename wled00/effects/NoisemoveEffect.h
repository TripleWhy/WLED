...
public:
    static constexpr const char metaData[] PROGMEM = "Noisemove@Move speed,Fade rate;!,!;!;01f;m12=0,si=0";
    ...
    bool nextFrameImpl(TransitionableParameters& parameters, const EffectCoordinate& coordinate) {
        ...
        int fadeoutDelay = (256 - parameters.speed) / 96;
        if ((fadeoutDelay <= 1 ) || ((parameters.call % fadeoutDelay) == 0)) buffer.fadeToBlackBy(4+ parameters.speed/4);

        uint8_t numBins = map(parameters.intensity,0,255,0,16);    // Map slider to fftResult bins.
