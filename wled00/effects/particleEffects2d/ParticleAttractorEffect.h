...
public:
    ...
    bool nextFrameImpl(TransitionableParameters& parameters, const EffectCoordinate& coordinate) {
        ...
        PartSys.setParticleSize(parameters.custom1 >> 1); //set size globally
        PartSys.setUsedParticles(map(parameters.intensity, 0, 255, 25, 190));

        if (parameters.custom2 > 0) // collisions enabled
            PartSys.enableParticleCollisions(true, map(parameters.custom2, 1, 255, 120, 255)); // enable collisions and set particle collision hardness
        else
        ...
        if (parameters.check2) {
            if ((parameters.call % 3) == 0) // move slowly
        ...
        if (UsermodManager::getUMData(&um_data, USERMOD_ID_AUDIOREACTIVE)) { // AR active, do not use simulated data
            ...
            strength = (parameters.speed * volumeSmth) >> 8;
        }
        #endif
        for (uint32_t i = 0; i < PartSys.usedParticles; i++) {
            PartSys.pointAttractor(i, attractor, strength, parameters.check3);
        }
