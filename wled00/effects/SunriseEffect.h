...
public:
    ...
    bool nextFrameImpl(TransitionableParameters& parameters, const EffectCoordinate& coordinate) {
        ...
        if (parameters.call == 0 || parameters.speed != previousSpeed) {
        ...
        if (parameters.speed > 120) { //quick sunrise and sunset
            unsigned counter = (strip.now >> 1) * (((parameters.speed -120) >> 1) +1);
            stage = triwave16(counter);
        } else if (parameters.speed) { //sunrise
            unsigned durMins = parameters.speed;
            if (durMins > 60) durMins -= 60;
            uint32_t s10Target = durMins * 600;
            if (s10SinceStart > s10Target) s10SinceStart = s10Target;
            stage = map(s10SinceStart, 0, s10Target, 0, 0xFFFF);
            if (parameters.speed > 60) stage = 0xFFFF - stage; //sunset
        }
