...
public:
    ...
    bool nextFrameImpl(TransitionableParameters& parameters, const EffectCoordinate& coordinate) {
        ...
        buffer.fade(SEGCOLOR(1), 224); // 6.25%
        ...
        for (size_t i=0; i<parameters.intensity/32+1U; i++) {
            ...
            buffer.setPixelColor(beatsin16_t(parameters.speed/4+i*2,0,coordinate.width-1), color_blend(SEGCOLOR(1), parameters.color_from_palette(strip.now/4+i*2, false, PALETTE_SOLID_WRAP, 0), my_sampleAgc));
        }
        return true;
    }
